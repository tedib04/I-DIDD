...
@ConstraintAutomaton.register('Alternate Precedence')
class AlternatePrecedenceConstraintAutomaton(ConstraintAutomaton, ABC):
    """
    Implements a Deterministic Finite Automaton (DFA) for the 'Alternate Precedence' constraint
    in the DECLARE process modeling language. This constraint requires that whenever
    the activation event (b) occurs, the target event (a) must have been before and no other (b) can occur in between.
    ...
    def __init__(self, constraint_name: str):
        super().__init__(constraint_name=constraint_name,
                         formula='((!(b))U(a)) && G((b) -> X[!]((!(b))U(a)) || G(!(b)))',
                         activation='b',
                         target='a')
        self.permanently_violating_state: int = 1
