...
@ConstraintAutomaton.register('Chain Precedence')
class ChainPrecedenceConstraintAutomaton(ConstraintAutomaton, ABC):
    """
    Implements a Deterministic Finite Automaton (DFA) for the 'Chain Precedence' constraint
    in the DECLARE process modeling language. This constraint requires that whenever
    the activation event (b) occurs, the target event (a) must have been executed immediately beforehand.
    ...
    `assets/dfa/images/chain_precedence.png`
    """
    ...
    def __init__(self, constraint_name: str):
        super().__init__(constraint_name=constraint_name,
                         ...
                         activation='b',
                         target='a')
        self.permanently_violating_state: int = 2
    ...
    def process_event(self,
        ...
        for token, state in active_constraints.items():
            if token.trace_id == trace_id:
                ...
                if constraint.first_activity not in seen_target:
    ...
    def update_constraint_state(self, constraint_state: Dict, next_state: Set, trace_id: str) -> None:
        ...
        # if state 2
        if current == self.permanently_violating_state:
            constraint_state['color'] = AutomatonColor.PERMANENTLY_VIOLATED
        else:  # if state 0 OR 1
