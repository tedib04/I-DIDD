...
@ConstraintAutomaton.register('Chain Response')
class ChainResponseConstraintAutomaton(ConstraintAutomaton, ABC):
    """
    Implements a Deterministic Finite Automaton (DFA) for the 'Chain Response' constraint
    in the DECLARE process modeling language. This constraint requires that whenever
    the activation event (a) occurs, the target event (b) must occur immediately afterward.
    ...
    `assets/dfa/images/chain_response.png`
    """
    ...
    def process_event(self,
        ...
        # If the current event is an activation of the constraint, generate a new triple (Constraint, Activation, Trace) and the corresponding automata.
        ...
        # Update automata instances for the current trace, using as input symbol the current event.
        for token, state in active_constraints.items():
            if token.trace_id == trace_id:

                # Map the event's activity to the placeholder expected by the automaton and move it
                self.move_automaton(
